...
#' @param replace.ref Replace the column name "Ref" with this argument when "Ref" is present
...
importCoreGenomeSNPs <- function(snps, snps.delim = ",", pos.col = "Pos",
    ...
    if (snpData.class == "character") {  # when a file name is provided
        ...
        # substitute the column name "Ref" with its genuine strain name when the Ref column is present and the substitution is specified
        if (is.character(replace.ref)) {
            i <- which(names(snps.core) == "Ref")  # length(i) = 0 when replace.ref = "" or "Ref" is not present
            if (replace.ref != "" & length(i) == 1) {  # length(i) actually equals one when the "Ref" column is found because R does not allow duplicated column names.
                print(paste0("Replacing the column name 'Ref' with ", replace.ref, "."))
                names(snps.core)[i] <- replace.ref
            } else {
                print("Warning: no column name is replaced as the value of replace.ref equals '' or the column name 'Ref' is absent in the SNP matrix.")
            }
        } else {  # default
            print("Warning: no column name is replaced for the reference strain because it is absent in the SNP matrix.")
        }
