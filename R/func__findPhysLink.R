#' @title Testing for physical linkage between every pair of bacterial genes at the allele level
#'
#' @description Find out physical linkage between bacterial genes at the allelic level with univariate linear mixed modesl.
#' This is the main function of this package. The term "sample" refers to either a bacterial isolate or a bacterial strain in this package.
#'
#' Dependent libraries: data.table, parallel, ape, phytools
#'
#' @param assoc.out An output list of the function lmms. It may not include the
#' large element snps for convenience. This parameter is used when users need to
#' incorporate distance information into the result of association analysis.
#' Other arguments, such as snps, snps.delim, ..., allelic.pam and genetic.pam,
#' etc., will not be used when this argument is valid.
#
...
#' @param outliers (optional) A vector of characters for isolate/strain names to be excluded from snps, pam and ds
...
#' @param max.dist (optional) An inclusive upper bound for filterring distance measurements. Measurements above this threshold will be ignored.
...
#' @param max.p (optional) The upper bound of P values to determine significance. P <= max.p will be called significant.
...
findPhysLink <- function(assoc.out = NULL,
    ...
    if (! .checkAssocOut(assoc.out)) {
        print("The argument assoc.out is not used as it is not a valid result of association analysis.")
        ...
        print("The argument assoc.out is a valid result of association analysis.")
        .checkDir(output.dir)
    }

    # Cannot use attach(assoc.out) to unpack the list due to name clashes with this function's arguments.
    stage.outputs <- .initialiseStageRecords("findPhysLink", save.stages, prefix)
    ...
    # 3. Infer identical-by-descent of measured distances and calculate slopes of the measurements ###############
    ...
    if (save.stages & file.exists(stage.record)) {
        assoc <- .recoverHistory(stage.record)  # It is very unlikely that a user wants to recover the result of this stage.
    } else {
    ...
    # 6. Construct the return object ###############
    out.list <- list(alleles = assoc.out[["alleles"]], genes = assoc.out[["genes"]],
                     mapping = assoc.out[["mapping"]], tests = assoc.out[["tests"]],
                     lmms.pat = assoc.out[["lmms.pat"]], lmms = assoc.out[["lmms"]],
                     struc = assoc.out[["struc"]],
                     ds = ds, lmms.ds = lmms.ds, ds.stats = ds.stats,
                     assoc = assoc, output.files = assoc.out[["outputs"]],
                     stage.outputs = stage.outputs)
    if ("snps" %in% names(assoc.out)) {
        out.list[["snps"]] <- assoc.out[["snps"]]
    }
