...
#  First edition: 11 Aug 2018, the latest edition: 14 Aug 2018
    ...
    if (years[1] > 0) {  # temporal mode
        ...
        for (yr in years) {
            ...
            if (n > 1) {  # multiple strains
                ...
                ws_yr <- ws[strains_yr]
                G[[as.character(yr)]] <- .countCoocurrencePerYear(E, yr, pam_yr, ds_yr, ws_yr)
        ...
        G <- .countCoocurrencePerYear(E = E, yr = "all", pam_yr = pam, ds_yr = ds, ws_yr = ws)
    }
...
.countCoocurrencePerYear <- function(E, yr, pam_yr, ds_yr, ws_yr) {
